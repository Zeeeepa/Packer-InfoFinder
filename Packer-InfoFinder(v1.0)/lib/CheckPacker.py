...
class CheckPacker():
    ...
    def checkHTML(self):
        ...
        if sslFlag == 1:
            # --- 优化：添加了健壮的超时设置 ---
            demo = requests.get(url=url, headers=headers, proxies=self.proxy_data, verify=False, timeout=(10, 30)).text
        else:
            # --- 优化：添加了健壮的超时设置 ---
            demo = requests.get(url=url, headers=headers, proxies=self.proxy_data, timeout=(10, 30)).text
